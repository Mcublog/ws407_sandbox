...
void Adafruit_PCD8544::updateBoundingBox(uint8_t xmin, uint8_t ymin,
  ...
  xUpdateMin = std::min(xUpdateMin, xmin);
  xUpdateMax = std::max(xUpdateMax, xmax);
  yUpdateMin = std::min(yUpdateMin, ymin);
  yUpdateMax = std::max(yUpdateMax, ymax);
}
 ...
 */
Adafruit_PCD8544::Adafruit_PCD8544(SPI_HandleTypeDef *theSPI)
    {
  spi_dev = theSPI;
}
...
void Adafruit_PCD8544::data(uint8_t c) {
  ...
  HAL_GPIO_WritePin(LCD_CE_GPIO_Port, LCD_CE_Pin, GPIO_PIN_RESET);
  HAL_SPI_Transmit(spi_dev, &c, 1, 10);
  HAL_GPIO_WritePin(LCD_CE_GPIO_Port, LCD_CE_Pin, GPIO_PIN_SET);
}
...
void Adafruit_PCD8544::display(void) {
  ...
  for (uint8_t page = (yUpdateMin / 8); page < (yUpdateMax / 8) + 1; page++) {
    ...
    HAL_GPIO_WritePin(LCD_CE_GPIO_Port, LCD_CE_Pin, GPIO_PIN_RESET);
    HAL_SPI_Transmit(spi_dev, pcd8544_buffer + (LCDWIDTH * page) + startcol, endcol - startcol + 1, 10);
    HAL_GPIO_WritePin(LCD_CE_GPIO_Port, LCD_CE_Pin, GPIO_PIN_SET);
  }
